...
wish = (function(){
  ...
  function characterizationMessage(expression, actual){
    var parsedExpression = expression.match(/(.*),\W*true/)[1];
    return `${parsedExpression} evaluated to ${handleComplexTypes(actual)}`;
  };
  ...
  function handleCharacterization(actual){
    var message = characterizationMessage(getTheExpression(), actual);
    var error = new Error(message);
  ...
  return function wish(assertion, characterizing = false){
    if (characterizing){
      throwIt(handleCharacterization(assertion));
    } else if (!(assertion)){
      throwIt(handleError(assertion));
    }
    else{
      return true;
    }
